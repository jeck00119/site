from services.algorithms.models.algorithm_parameters_model import AlgorithmParametersModel


class ContourLogicAlgorithmModel(AlgorithmParametersModel):
    lower_threshold: int = 0
    upper_threshold: int = 0

    gamma: float = 0.01

    kernel_erode_size: int = 0
    erode_iterations: int = 0

    kernel_dilate_size: int = 0
    dilate_iterations: int = 0

    kernel_blur: int = 0

    part_area_min: float = 0.0
    part_area_max: float = 0.0
    line_size: int = 1

    graphics: list = [
        {
            "color": "rgba(140, 235, 52, 0.5)",
            "rotation": 0,
            "bound": [
                10,
                10,
                100,
                100
            ],
            "offset": [
                0,
                0
            ],
            "rect": [
                10,
                10,
                100,
                100
            ],
            "masks": [],
            "masksColors": []
        }
    ]
    golden_position: list = [0, 0]

    def get_ui_from_type(self):
        return self.load_ui_dictionary("services/algorithms/implementation/ui_objects/contour_logic.json")
