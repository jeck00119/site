...
export default {
    ...
    async removeConfiguration(context, payload) {
        ...
        if(!response.ok)
        {
            const error = new Error(responseData?.detail || `Failed to remove configuration with id: ${payload.uid}!`);
            throw error;
        }
        else
        {
    ...
    async editConfiguration(context, payload) {
        ...
        if(!response.ok)
        {
            const error = new Error(responseData?.detail || `Failed to edit configuration with id: ${payload.uid}!`);
            throw error;
        }
        else
        {
    ...
    async addConfiguration(context, payload) {
        ...
        if(!response.ok)
        {
            const error = new Error(responseData?.detail || `Failed to add new configuration: ${payload.name}!`);
            throw error;
        }
        else
        {
    ...
    async copyConfiguration(context, payload) {
        ...
        if(!response.ok)
        {
            const error = new Error(responseData?.detail || `Failed to copy configuration: ${payload.config.name}!`);
            throw error;
        }
        else
        {
    ...
    async resetAllDatabases() {
        const { response, responseData } = await post(`http://${ipAddress}:${port}/configurations/reset_all_dbs`, null);

        if(!response.ok)
        {
            const error = new Error(responseData?.detail || `Failed to reset the databases!`);
            throw error;
        }
    },
    ...
    async closeConfigurationChangedSocket(_, payload) {
        const { response, responseData } = await post(`http://${ipAddress}:${port}/configurations/${payload.uid}/ws/close`);

        if(!response.ok)
        {
            const error = new Error(responseData?.detail || `Failed to close configuration changes notifier socket!`);
            throw error;
        }
    }
}